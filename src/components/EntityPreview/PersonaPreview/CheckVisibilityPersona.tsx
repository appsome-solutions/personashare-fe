...
export const CheckVisibilityPersona = () => {
  ...
  const NumberOfVisibilityList = showMore ? data?.persona.networkList.length : 4;

  const CheckVisibilityList = () => {
    if (!data?.persona?.visibilityList) {
      return (
        <>
          <HowManyUsers>0 persons saved your profile</HowManyUsers>;
          <ComponentWithTable>
            {data?.persona.visibilityList.slice(0, NumberOfVisibilityList).map((persona: AgregatedPersona) => (
      ...
      return (
        <>
          <HowManyUsers>{data?.persona?.visibilityList?.length} persons saved your profile</HowManyUsers>
          <ComponentWithTable>
            {data?.persona.visibilityList.slice(0, NumberOfVisibilityList).map((persona: AgregatedPersona) => (
              <PersonaCardComponent key={persona.uuid}>
                <PersonaCardMini
